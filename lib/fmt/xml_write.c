...
usage(FILE *stream, const char *progname)
{
    return 
        fprintf(
            stream, 
            "Usage: %s [OUTPUT_OPTS [INPUT [OUTPUT]]]\n"
            "Convert a HID report descriptor from native to XML format.\n"
...
main(int argc, char **argv)
{
    ...
    const char        **argp_list[]     = {&output_options,
                                           &input_name,
                                           &output_name};
    size_t              i;
     ...
     * Open output stream
     */
    output = hidrd_snk_new_opts(&hidrd_xml_snk,
